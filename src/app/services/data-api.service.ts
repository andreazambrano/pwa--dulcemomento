...
export class DataApiService {
  ...
  //   const url_api = `http://localhost:3021/api/tixes/${tixI}/?access_token=${token}`;
  //   return this.http
  ...
  // }
  	...
  	getBookPending(){	
		const url_api='http://localhost:3021/api/book?filter[where][status]=pending';
		return (this.books = this.http.get(url_api));
	}
	getBookConf(){	
		const url_api='http://localhost:3021/api/book?filter[where][status]=Confirmada';
		return (this.books = this.http.get(url_api));
	}
	getValidationPending(){
		const url_api='http://localhost:3021/api/validations?filter[where][status]=pending';
		return (this.books = this.http.get(url_api));
	}


	getAllTixs(){
		const url_api = 'http://localhost:3021/api/tixes?filter[where][status]=activated';
	...
	getTixById(id:string){
		//console.log(id);
		let indice = id;
		const url_api=`http://localhost:3021/api/tixes/${indice}`;
	...
	getCardById(id:string){
		let indice = id;
		const url_api=`http://localhost:3021/api/card/${indice}`;
		this.card = this.http.get(url_api);
		return (this.card);
	}
	getBookById(id:string){
		let indice = id;
		const url_api=`http://localhost:3021/api/book/${indice}`;
		this.book = this.http.get(url_api);
		return (this.book);
	}
	getInfo(){
		// let indice = id;
		const url_api=`http://localhost:3021/api/infos/`;
		this.info = this.http.get(url_api);
		return (this.info);
	}
	getPending(){
		const url_api='http://localhost:3021/api/tixes?filter[where][status]=pending';
		return (this.tixs = this.http.get(url_api));
	}
	getUsersPending(){
		const url_api='http://localhost:3021/api/card?filter[where][status]=pending';
		return (this.cards = this.http.get(url_api));
	}

	getActivePartners(){
		const url_api='http://localhost:3021/api/card?filter[where][and][0][status]=active&filter[where][and][1][type]=partnerType';
		return (this.cards = this.http.get(url_api));
	}
	getPendingPartners(){
		const url_api='http://localhost:3021/api/card?filter[where][and][0][status]=pending&filter[where][and][1][type]=partnerType';
		return (this.cards = this.http.get(url_api));
	}

	getActiveAffiliates(){
		const url_api='http://localhost:3021/api/card?filter[where][and][0][status]=active&filter[where][and][1][type]=affiliateType';
		return (this.cards = this.http.get(url_api));
	}
	getPendingAffiliates(){
		const url_api='http://localhost:3021/api/card?filter[where][and][0][status]=pending&filter[where][and][1][type]=affiliateType';
		return (this.cards = this.http.get(url_api));
	}
	saveTixFree(tix :TixInterface){
	//	let token = this.authService.getToken();
		const url_api='http://localhost:3021/api/tixes';
		return this.http
	...
	saveTix(tix :TixInterface){
		let token = this.authService.getToken();
		const url_api='http://localhost:3021/api/tixes?access_token${token}';
		return this.http
	...
	updateTix(tix :TixInterface, id: string){
		let token = this.authService.getToken();
		const url_api=`http://localhost:3021/api/tixes/${id}/?access_token$={token}`;
		return this.http
	...
	updateBook(book :BookInterface, id: string){
		let token = this.authService.getToken();
		const url_api=`http://localhost:3021/api/book/${id}/?access_token$={token}`;
		return this.http
	...
	updateValidation(validation :ValidationInterface, id: string){
		let token = this.authService.getToken();
		const url_api=`http://localhost:3021/api/validations/${id}/?access_token$={token}`;
		return this.http
	...
	getCards(id:string){
		let indice = id;
		const url_api = "http://localhost:3021/api/card?filter[where][userd]=a"+indice;
		return this.http.get(url_api);
	}
	saveCard(card: CardInterface){
		//const token = this.authService.getToken();
		//console.log(token);
		const url_api='http://localhost:3021/api/card';
		return this.http
		.post<CardInterface>(url_api, card)
		.pipe(map(data => data));
	}
	saveBook(book: BookInterface){
		const url_api='http://localhost:3021/api/book';
		return this.http
		.post<BookInterface>(url_api, book)
		.pipe(map(data => data));
	}
	saveValidation(validation: ValidationInterface){
		const url_api='http://localhost:3021/api/validations';
		return this.http
